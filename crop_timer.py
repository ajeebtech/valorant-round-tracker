...
class TimerCropper:
    ...
    def detect_timer_region_heuristic(self, image: np.ndarray) -> Tuple[int, int, int, int]:
        ...
        center_x = w // 2
        timer_width = int(w * 0.06)   # Reduced width to focus only on timer digits
        timer_height = int(h * 0.05)  # Reduced height to exclude elements below
        timer_y = int(h * 0.02)       # Start slightly lower from top edge
